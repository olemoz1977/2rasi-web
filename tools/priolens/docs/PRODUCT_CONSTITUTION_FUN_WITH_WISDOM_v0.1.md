...
PrioLens should let a person notice patterns in their **first, low-deliberation visual choices** without turning the experience into a questionnaire, interview or self-analysis exercise.
...
The participant should be able to finish the core experience in approximately **2–4 minutes**. This is a product target, not yet an empirically fixed threshold.

Initial interaction budget: preferably **12–18 meaningful choices**, with 20 as a soft ceiling for the default public flow. A longer research protocol may exist separately.
...
The public product should consume only mappings and stimuli that survived research. The user should not be asked to perform the researchers' validation work.
...
> `Šioje sesijoje tavo pirmi pasirinkimai dažniau krypo į ...`

Potentially useful result components after validation:

- 2–3 strongest recurring choice tendencies;
- one meaningful tradeoff/tension;
- one surprising inconsistency, cycle or context sensitivity if present;
- a short reflective question as an optional final layer, not a requirement to complete the tool.

Avoid:

- `Tu esi ...`;
- `Tavo pasąmonė nori ...`;
- `Tavo tikroji vertybė yra ...`;
...
- 28 edges remain a research/comparator option;
- 12–18 trials are a product UX target, not a validated measurement minimum;
